...
class SdfWriter:
    ...
    def write(self, smi, mol, mol_id, existing_props, new_props, smiles_prop_name=None):
        del existing_props
        if not mol:
...
class SmilesWriter:
    ...
    def write(self, smi, mol, mol_id, existing_props, new_props, smiles_prop_name=None):
        ...
        if type(mol_id) == str and self.sep in mol_id:
            mol_id = '"' + mol_id + '"'
        ...
        try:
            ...
            if self.id_col:
                if mol.HasProp(self.id_col):
                    mol_id = mol.GetProp(self.id_col)
                else:
                    mol_id = None
            else:
                mol_id = None
...
class SmilesReader:
    ...
    def __init__(
        self, input_file, read_header, delimiter, id_column, mol_column, recs_to_read
    ):
        ...
        tmp_reader, file_reader = self.create_readers(input_file)
        # read header line
        if read_header:
        ...
        self.reader, self.file = self.create_readers(input_file)
        if read_header:
            line = next(self.reader)

    def create_readers(self, input_file):
        if input_file.endswith(".gz"):
            r = gzip.open(input_file, "rt")
            return csv.reader(r, delimiter=self.delimiter), r
        else:
            r = open(input_file, "rt", encoding="utf-8")
            return csv.reader(r, delimiter=self.delimiter), r
...
def create_reader(
    ...
    if filetype is None:
        if (
            ...
            or filename.endswith(".sdf.gz")
            or filename.endswith(".sd")
            or filename.endswith(".sd.gz")
        ):
            filetype = "sdf"
        else:
            filetype = "smi"
...
def get_num_sp3_centres(mol):
    return sum(
        (x.GetHybridization() == Chem.HybridizationType.SP3) for x in mol.GetAtoms()
    )
...
def rdk_read_single_mol(input_file):
    ...
    if input_file.endswith(".mol"):
        ...
        raise ValueError(
            "Unsupported file type. Must be .mol .sdf or .sdf.gz. Found " + input_file
        )
    return mol
            ...
            if token.endswith(".mol"):
                ...
                for i, m in enumerate(ms):
                    if m:
                        ...
                        DmLog.emit_event(
                            "WARNING: could not process molecule", i, "from", token
                        )
    return mols
...
def rdk_mol_supplier(input_file):
    ...
    if input_file.endswith(".sdf"):
        ...
        raise ValueError(
            "Unsupported file type. Must be .sdf or .sdf.gz. Found " + input_file
        )
    return suppl
...
def check_molecules_are_3d(input_file, num_to_check=10):
    if input_file.endswith(".gz"):
        suppl = Chem.ForwardSDMolSupplier(gzip.open(input_file))
    else:
        suppl = Chem.ForwardSDMolSupplier(input_file)
